...
int parse_async_payload(const char * buf, const int len, struct CMD_ASYNC_TX * const cmd)
{
	...
	for (int k = 0; k < 5; k++)
	{
		...
		int chan = strtoul((char *)&channel[0], NULL, 16);
		if ((0 > chan) | (chan > 5))
		{
		...
		int app_hash = strtoul((char *)app, NULL, 16);

		/*
		 * Assign the hashs to the outputs
		 */
		cmd->appSliders[k] = app_hash;
	}
...
int build_async_payload(const struct CMD_ASYNC_RX * const cmd, char *buf, int * len)
{
	...
	*len = snprintf(buf,
			...
			cmd->sliders[1].appSlider,
			cmd->sliders[1].appMatch,
			cmd->sliders[2].appSlider,
			cmd->sliders[2].appMatch,
			cmd->sliders[3].appSlider,
			cmd->sliders[3].appMatch,
			cmd->sliders[4].appSlider,
			cmd->sliders[4].appMatch
			);
	return 0;
}
