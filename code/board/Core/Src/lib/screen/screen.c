...
struct BufferRequest* draw_buffer(struct ScreenOrder const *cmd)
{
	...
	case FULL:
		...
		// Compute the selective refresh parameters
		__rval.address = ICON_START;
		__rval.len = (ICON_LEN * 4) + (VOLUME_LEN * 4) + (SPEAKER_LEN * 4) + (MUTE_LEN * 4);
		break;
	case VOLUME:
		_draw_volume(&__rval);
		_draw_status(&__rval);
		...
		// Compute the selective refresh parameters
		__rval.address = VOLUME_START;
		__rval.len = (VOLUME_LEN * 4) + (SPEAKER_LEN * 4);
		break;
	case STATUS:
		_draw_mute(&__rval);

		// Compute the selective refresh parameters
		__rval.address = MUTE_START;
		__rval.len = (MUTE_LEN * 4);
		break;
	case VOLSTAT:
		_draw_volume(&__rval);
		_draw_status(&__rval);
		...
		// Compute the selective refresh parameters
		__rval.address = VOLUME_START;
		__rval.len = (VOLUME_LEN * 4) + (SPEAKER_LEN * 4) + (MUTE_LEN * 4);
	}
